...
pub fn dispatch(
    ...
    ckb_testkit::info!(
        "dispatch with params --n-users {} --cells-per-user {} --capacity-per-cell {} --loop-times {}",
        users.len(),
        cells_per_user,
        capacity_per_cell,
        loop_times
...

    {
        ...
        let need_capacity = (users.len() as u64 * cells_per_user * capacity_per_cell + total_fee)*loop_times;
        assert!(
